...
import { buildPrivacyFromDraft, filterPrivacyPickableContacts } from './privacySelectionUtils'
...
export function PrivacySettingsModal({
  ...
  const pickableContacts = useMemo(() => filterPrivacyPickableContacts(contacts), [contacts])
  ...
  const [activeTab, setActiveTab] = useState<'tags' | 'contacts'>('tags')
      ...
      {open ? (
        ...
        >
          <motion.div
            ...
            className="relative flex max-h-[85vh] w-[90%] max-w-sm flex-col overflow-hidden rounded-3xl bg-white shadow-[0_20px_60px_rgba(0,0,0,0.08)]"
                ...
                {view === 'main' ? (
                  ...
                  >
                    ...
                    <div className="border-t border-gray-100 px-6 py-3">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {!needsAudience ? (
                          <span className="text-[10px] text-[#D1D5DB]">选「部分可见/不给谁看」后可用</span>
                        ) : null}
                      </div>
                              ...
                              <div key={tag.id} className="border-b border-gray-50 last:border-0">
                                <div className="flex items-center gap-2 py-3">
                                  {needsAudience ? (
                                    ...
                                    <div className="flex min-w-0 flex-1 items-center gap-3 px-0.5">
                                      <span className="truncate text-[15px] text-[#111827]">{tag.name}</span>
                                      <span className="shrink-0 text-[11px] text-[#D1D5DB]">
                                        {members.length ? `${members.length} 人` : '暂无成员'}
                                      </span>
                                  ...
                                  {expanded ? (
                                    ...
                                    >
                                      <div className="flex gap-3 overflow-x-auto pb-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
                                        {members.length ? (
                                          members.map((member) => (
                                            <div key={member.id} className="w-14 shrink-0 text-center">
                      ...
                      {needsAudience ? (
                        ...
                        >
                          <div className="border-b border-gray-100 px-6">
                            <button
                              type="button"
                              onClick={() => setActiveTab(activeTab === 'contacts' ? 'tags' : 'contacts')}
                  ...
                  <CreateTagPanel
                    ...
                    onSave={(tag) => {
                      ...
                      if (draftMode === 'public' || draftMode === 'private') {
