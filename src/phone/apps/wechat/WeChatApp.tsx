...
import { requestWeChatPeerReplyBubbles, type ChatTranscriptTurn } from './wechatChatAi'
...
function WeChatAppInner({ onBack }: Props) {
  ...
  const buildFriendRequestAiReply = useCallback(
    async (params: { characterId: string; messages: FriendRequest['messages']; replyBias?: string }) => {
      ...
      const longTermMemoryNotes = await personaDb.formatCharacterMemoriesForPrompt(character.id)
  ...
  const triggerNewFriendRequestReply = useCallback(
    ...
    [buildFriendRequestAiReply, pendingNewFriendRequests, playerIdentityId, refreshPendingNewFriendRequests],
  )
