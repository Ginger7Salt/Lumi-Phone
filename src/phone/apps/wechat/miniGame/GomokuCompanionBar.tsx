...
export function GomokuCompanionBar({
  playerAvatarUrl,
  charAvatarUrl,
  charName,
  reactionText,
  ...
  const totalStones = playerStoneCount + charStoneCount
  const isPlayerTurn = playerGoesFirst ? totalStones % 2 === 0 : totalStones % 2 === 1
  const playerTurnHint = gameOver ? '' : aiThinking ? '等待落子' : isPlayerTurn ? '轮到你' : '等待对方'
  const charTurnHint = gameOver ? '' : aiThinking ? '思考中…' : isPlayerTurn ? '等待对方' : '轮到 TA'
    ...
    <div className="pointer-events-none flex w-full max-w-[min(92vw,420px)] flex-col items-center">
      <div className="flex items-start justify-center gap-10 sm:gap-14">
        <div className="flex flex-col items-center">
          ...
          >
            {playerTurnHint || `${playerStoneCount} 子`}
          </div>
