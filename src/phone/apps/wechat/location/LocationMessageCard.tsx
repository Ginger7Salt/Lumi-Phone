...
/** 聊天流中的坐标覆写位置卡片 · Soft Minimalist */
export function LocationMessageCard({ data, compact = false }: Props) {
    ...
    >
      <div className="relative h-[96px] w-full overflow-hidden bg-[#e8e8e8]">
        ...
        <div className="absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2">
          <span className="block size-3 rounded-full bg-[#D4AF37] ring-4 ring-[#D4AF37]/30" aria-hidden />
        </div>
      </div>
      <div className="bg-white px-3.5 py-3">
        <p className="truncate font-serif text-[15px] leading-snug text-[#0a0a0a]">{name || 'TARGET'}</p>
        {address ? (
          <p className="mt-1 truncate text-[10px] leading-relaxed text-gray-400">{address}</p>
        ) : (
        ...
        <div className="mt-3 border-t border-dashed border-gray-200 pt-2.5">
          ...
          <p className="mt-0.5 font-mono text-[11px] tracking-wide text-[#8B7355]">[ {distance || 'Unknown'} ]</p>
        </div>
      </div>
    </motion.div>
  )
