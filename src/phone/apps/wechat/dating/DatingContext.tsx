...
import {
  ...
  DATING_AI_SCENE_HINTS_PROMPT_MAX,
} from './types'
...
function aiPlotPersistFields(
  ...
  return initialAiPlotVersions(parsed.content, parsed.logicPass || undefined, parsed.planSummary)
}
...
`
  ...
  const recentPrivRefBlock = onlineCtx?.recentPrivateAiRoundsBlock?.trim()
  const recentOffRefBlock = onlineCtx?.recentOfflineAiRoundsBlock?.trim()
  ...
  const recentPrivRefClipped = clipDatingReferenceTail(recentPrivRefBlock ?? '', refCap, '最近私聊参考')
  const recentOffRefClipped = clipDatingReferenceTail(recentOffRefBlock ?? '', refCap, '最近线下参考')
  let unsOffClipped = ''
  ...
  const playerInputIntentRule =
    godPerspective && trimmedUserForReminder.length > 0
      ? `【上帝视角·输入边界（最高优先级）】下列仅为屏外剧情引导；玩家本人**不在本轮画面**。**禁止**描写玩家出场、在场、与${character.realName}/NPC 当面互动、引号对白或肢体动作。须写 ${character.realName}/NPC 在玩家**看不见**处的独处、与他人互动，或隔空侧面提及（看手机、想起你、发消息等）。\n`
  ...
  const userPromptRaw =
        ...
        `【本轮承接范围】优先承接${
          godPerspective
            ...
            : playerInputIntentMode === 'paraphrase'
              ? '导演指令'
              : '“玩家输入：...”'
        }中的意图，同时必须回接最近剧情至少 1 个未收束点，保持连续性；${
          godPerspective
            ? '**本轮须维持玩家不在场的屏外镜头**'
            : mainCharacterOffstage
        ...
        `最近私聊原文参考（最近 6 轮角色回复及用户输入；可能已总结，仅供承接口吻）：\n${recentPrivRefClipped || '（暂无）'}\n\n` +
        `最近线下剧情参考（最近 6 轮 AI 剧情及玩家输入；可能已总结，仅供承接口吻）：\n${recentOffRefClipped || '（暂无）'}\n\n` +
...
export function DatingProvider({ children }: { children: ReactNode }) {
  ...
  const getOnlineMemoryContext = useCallback(
    async (
      ...
      let recentPrivateAiRoundsBlock = ''
      let recentOfflineAiRoundsBlock = ''
      if (convKey && !convKey.startsWith('wxgrp:')) {
        ...
        try {
          recentPrivateAiRoundsBlock = convKey.startsWith('wxapriv:')
            ? await formatRecentAiRoundsPrivateChatReferenceBlock({ conversationKey: convKey })
            : await formatRecentAiRoundsPrivateChatByCharacter({ characterId: cid })
        } catch {
          recentPrivateAiRoundsBlock = ''
        }
        try {
          const anchorGroupId =
      ...
      try {
        recentOfflineAiRoundsBlock = await formatRecentOfflinePlotsAiRoundsReference(
          cid,
          chRow?.name?.trim() || chRow?.wechatNickname?.trim() || null,
        )
      } catch {
        recentOfflineAiRoundsBlock = ''
      }
