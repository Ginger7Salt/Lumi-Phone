...
function buildSlimDatingPlotChatMessages(params: {
  ...
  const inputLabel = params.godPerspective ? '屏外剧情引导' : '玩家输入'
...
`
  ...
  const userReactionSlimHint = summarizeUserReactionForSlimRetry({ autoUserReaction, godPerspective })
  ...
  const systemPromptRaw =
    `${charUserDirective}${DATING_STYLE_SYSTEM_PROMPT}${styleAppend}${
      datingArchiveBlock
        ...
        : '\n'
    }\n${combinedMemNote}\n\n${wbAfterBlock}${FICTIONAL_COT_APPENDIX}\n\n${PROSE_FORBIDDEN_LEXICON_PROMPT}`
  ...
  const userPromptRaw =
        ...
        `【本轮承接范围】优先承接${
          ...
          godPerspective ? '**本轮须维持玩家不在场的屏外镜头**' : ''
        }。\n` +
      ...
      void (async () => {
        ...
        try {
          ...
          dispatchDatingPlotGenerationError({
            characterId: charId,
            characterName: char.realName,
            message: e instanceof Error ? e.message : '剧情生成失败',
          })
        } finally {
          endDatingPlotContentHint(charId)
    ...
    async (
      ...
      try {
        ...
        dispatchDatingPlotGenerationError({
          characterId: charId,
          characterName: char.realName,
          message: e instanceof Error ? e.message : '重新生成失败',
        })
      } finally {
