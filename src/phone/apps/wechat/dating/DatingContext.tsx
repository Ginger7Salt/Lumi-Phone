...
1) 旁白：只写正文，不加姓名前缀；提及玩家时只写姓名或第三人称，**禁止**写「${userDisplayName}（你）」或任何「某某（你）」。
2) 对白：必须使用「姓名：内容」格式。
   - 若说话者是玩家，必须写「${userDisplayName}（你）：内容」。
   - 其他角色一律写真实姓名（例：「${character.realName}：内容」）。
   - **「（你）」仅用于玩家对白行首标签（最高优先级）**：「（你）」**只能**出现在**玩家本人**那一行的说话人前缀里（\`${userDisplayName}（你）：\`），供界面姓名框使用。**禁止**在 NPC 对白正文、旁白、内心 OS、闪回正文里写出「${userDisplayName}（你）」——他人称呼或提及玩家时只写「${userDisplayName}」或合理称谓（如小名、同学），**勿**加「（你）」；否则会出现「纪旌：${userDisplayName}（你），……」这种把界面标签写进台词的穿帮。
   - **单气泡单说话人（最高优先级·防串台）**：一行里**只允许出现一套**「姓名：」作为该行主前缀；冒号后的**正文里禁止**再写第二套「某某：」「某某（你）：」来开头对白——否则界面会把整行归到第一个姓名下，出现**用户的话显示在 NPC 气泡里**的穿帮。
   - 玩家与 NPC 轮替发言时**必须分行**：先写一行「${userDisplayName}（你）：……」，需要 NPC 再接一行「${character.realName}：……」。**严禁**写成「${character.realName}：${userDisplayName}（你）：……」这种**单行双前缀**。
3) **一行一个气泡**：换行即新气泡；是否在约 25 字处换行、在何种句读处换行，**完全由你在正文里用换行符决定**；前端**不会**再按字数、标点或句读自动拆条。
4) **每行长度建议**：每行（每气泡）约 **22～30 字**（一字一算，可略浮动），须保持**整句与语意完整**；除非刻意短句强调，避免通篇大量「整行只有几个字」的气泡。
5) **对白折行铁律（最高优先级·防穿帮）**：同一人一段话若拆成多行，**每一行都必须单独写「姓名：」前缀**（例：上一行 \`纪旌：可爱这种词，跟我这种，\` 下一行须写 \`纪旌：整天想翻墙逃课的人，好像不沾边吧？\`）。**禁止**只写后半句而无「姓名：」——无前缀行会被系统判为**旁白**，造成「上句对白、下句变灰」的灾难性错判。
6) 禁止序号、禁止 Markdown、禁止输出任何格式说明。
...
8) 闪回段内容形态与普通剧情一致：必须同时包含「旁白推进 + 角色对白 + 心理描写」三类信息。
