...
const STYLE_HINT =
  '旁白直写；对白只能用双引号"..."；内心OS：仅用「英文双星号」成对包裹整句（与界面插入按钮一致）；旁白轻吐槽、画外俏皮话：普通旁白直写，禁止用双星号包裹，以免被渲染成内心OS。' +
...
async function generateDatingAi(
  ...
  opts: { godPerspective: boolean; perspective: NarrativePerspective },
...
【推进落点卡】锚点+衔接；动作→连锁；**我…** OS 至少1句。本分册·必查：是。
...
"${seed.slice(0, 24)}。"他低声接住这个话题，语气平稳。`
  ...
  const { godPerspective, perspective } = opts
  ...
  const perspectiveRule = godPerspective
    ? `人称要求（本轮·上帝视角）：旁白以第三人称（他/她/${character.realName}等）写约会对象与在场他人；**禁止**旁白用「你」指${character.realName}或其动作；「你」仅明确指玩家时使用；角色内心 OS 仍为「我…」（我=${character.realName}）。`
    : perspective === 'first'
  ...
  const branchHintBlock =
    genOptions?.branchContinuationHint?.trim() ?
      ...
      : ''
  const targetCharsRaw = Number(genOptions?.lengthTargetChars ?? 180)
  const targetChars = Number.isFinite(targetCharsRaw) ? Math.max(60, Math.min(600, Math.round(targetCharsRaw))) : 180
  ...
  /** 单次补全上限（动态收紧）：避免极端长输出导致高延迟。 */
  const maxTokens = Math.min(4200, Math.max(1400, Math.round(targetChars * 10)))
  const requestPromise = openAiCompatibleChat(apiConfig as any, messages, { temperature: 0.82, max_tokens: maxTokens })
  const timeoutPromise = new Promise<string>((_, reject) => {
    window.setTimeout(() => reject(new Error('剧情生成超时（>65s），请重试或降低字数目标。')), 65000)
  })
...
export function DatingProvider({ children }: { children: ReactNode }) {
    ...
    async ({ bias, perspective = 'second', genOptions }: { bias: string; perspective?: NarrativePerspective; genOptions?: NarrativeGenOptions }) => {
      ...
      try {
        ...
        const aiTextRaw = await generateDatingAi(
          ...
          undefined,
          { godPerspective: currentArchive.godPerspective, perspective },
  ...
  const regenerateAiPlot = useCallback(
    async (plotId: string, perspective: NarrativePerspective = 'second', genOptions?: NarrativeGenOptions) => {
