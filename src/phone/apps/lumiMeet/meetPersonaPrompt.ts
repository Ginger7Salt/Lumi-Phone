...
export const MEET_NINE_DIMENSION_JSON_SCHEMA = `
...
维度说明（生成时须落实，禁止空壳）：
一 base：**info** 写外貌体征、穿搭气质、小动作、癖好（开头用一句年龄自述 "×× 岁。"，数字须与顶层 age 完全一致；**不要**在这里用 MBTI 解释职业）。**realName** 与顶层 realName 一致（真实姓名）。**birthdayMD** 为公历月日 \`MM-DD\`（须与 age、学生/职场身份常识一致）。**weightKg** 为千克数字字符串（如 "52"），与身形描写匹配、忌夸张。**zodiac** 写中文星座名（可与生日一致）。**physiology** 写体态动作习惯。
...
- 除下条外，comprehensive 各 prose 及 persona：指本人写 {{char}}；指 "当前滑动用户 / 匹配对象" 且属于**可变、随会话延展**的语境时可用 {{user}}（勿写真实昵称汉字）。
...
- nickname 仅用于界面展示，勿用它顶替正文里的 {{char}}。
- 自然口语自称 "我" 可以保留。
